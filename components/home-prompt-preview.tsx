    ...
    <section className="bg-white">
      <div className="mx-auto max-w-6xl px-4 py-16 sm:px-6 sm:py-20 lg:px-8">
        ...
        <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
          ...
          <Link
            href="/library"
            className="inline-flex items-center gap-2 rounded-full border border-neutral-200 bg-white px-4 py-2 text-sm font-semibold text-neutral-700 shadow-sm transition-all hover:border-neutral-300 hover:bg-neutral-50 hover:shadow"
          >
            View all prompts
    ...
    <article className="group flex flex-col overflow-hidden rounded-2xl border border-neutral-200 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-violet-200 hover:shadow-xl hover:shadow-neutral-200/60">
      <div className="relative aspect-[4/3] bg-gradient-to-br from-violet-100 via-fuchsia-50 to-pink-100">
        {prompt.image_url ? (
          ...
          <ImageIcon className="absolute left-1/2 top-1/2 h-8 w-8 -translate-x-1/2 -translate-y-1/2 text-violet-300" />
        )}
        <div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
      </div>
      <div className="flex flex-1 flex-col p-5">
        <p className="line-clamp-1 text-sm font-bold text-neutral-900">{prompt.title}</p>
        <p className="mt-1 text-xs text-neutral-500">{prompt.category}</p>
        <div className="mt-auto pt-4">
          <Link
            href="/library"
            className="inline-flex items-center gap-1.5 text-xs font-bold text-violet-600 transition-colors hover:text-violet-800"
          >
            Open &amp; copy
  ...
  return (
    ...
    >
      <div className={`relative aspect-[4/3] bg-gradient-to-br ${gradient}`}>
        <ImageIcon className="absolute left-1/2 top-1/2 h-8 w-8 -translate-x-1/2 -translate-y-1/2 text-violet-400 transition-transform duration-300 group-hover:scale-110" />
      </div>
      <div className="flex flex-1 flex-col p-5">
        <p className="text-sm font-bold text-neutral-900">{title}</p>
        <p className="mt-1 text-xs text-neutral-500">Open the prompt library</p>
        <div className="mt-auto pt-4">
          <span className="inline-flex items-center gap-1.5 text-xs font-bold text-violet-600 transition-colors group-hover:text-violet-800">
            Explore prompts
