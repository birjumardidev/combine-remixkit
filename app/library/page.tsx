  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <main className="mx-auto max-w-[1600px] px-3 pb-20 pt-4 sm:px-4 md:px-6 lg:px-8">
        {/* Category pills */}
        <div className="sticky top-[64px] z-30 -mx-3 mb-5 bg-white/95 px-3 py-3 backdrop-blur-xl border-b border-neutral-100/80 sm:-mx-4 sm:px-4 md:-mx-6 md:px-6 lg:-mx-8 lg:px-8">
            ...
            {FILTER_PILLS.map((pill) => {
              ...
              return (
                <button
                  ...
                  className={cn("pill", active ? "pill-active" : "pill-inactive")}
                >
                  {pill.label}
                </button>
              );
        ...
        {error ? (
          <div className="mx-auto max-w-md rounded-2xl border border-red-200 bg-red-50 p-10 text-center">
            <p className="text-sm font-semibold text-red-700">Couldn&apos;t load prompts.</p>
    ...
    <header className="sticky top-0 z-40 border-b border-neutral-200/60 bg-white/90 backdrop-blur-xl">
      <div className="mx-auto flex max-w-[1600px] items-center gap-3 px-3 py-3 sm:gap-4 sm:px-4 md:px-6 lg:px-8">
        ...
        <Link href="/" className="flex shrink-0 items-center gap-2.5 group">
          <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-neutral-900 shadow-md shadow-violet-500/25 transition-all duration-300 group-hover:scale-105">
            <Wand2 className="h-[17px] w-[17px] text-white" />
          </span>
          <span className="hidden text-[1.05rem] font-bold tracking-tight text-neutral-900 sm:block">
            RemixKit
          </span>
        </Link>
        ...
        <div className="relative mx-auto w-full max-w-2xl flex-1">
          <Search className="pointer-events-none absolute left-4 top-1/2 h-[17px] w-[17px] -translate-y-1/2 text-neutral-400" />
          <input
            value={query}
        ...
        <Link href="/prompts" className="btn-secondary hidden shrink-0 sm:inline-flex">
          Prompt Guides
        </Link>
        <Link href="/generate" className="btn-primary shrink-0">
...
function PinCard({
  ...
  return (
    <article className="pin-card group masonry-item">
      <div className="pin-image-wrap shadow-sm transition-shadow duration-300 group-hover:shadow-md">
        <button
          type="button"
          onClick={onOpen}
        ...
        <div className="pin-overlay hidden sm:flex pointer-events-none">
          ...
          <div>
            ...
            <span className="mt-2 inline-flex items-center gap-1 rounded-full bg-white/95 px-2.5 py-1 text-xs font-semibold text-neutral-900 shadow-sm">
...
function PinModal({
  ...
  return (
    ...
    >
      <div
        className="relative flex w-full max-w-[360px] flex-col overflow-hidden rounded-2xl bg-white shadow-2xl ring-1 ring-neutral-200/50 sm:max-h-[min(88vh,500px)] sm:max-w-[640px] sm:flex-row md:max-w-[720px]"
      ...
      >
        ...
        <button
          type="button"
          onClick={onClose}
          className="absolute right-3 top-3 z-10 flex h-8 w-8 items-center justify-center rounded-full bg-black/40 text-white backdrop-blur-md transition hover:bg-black/60"
          aria-label="Close"
        >
        ...
        {/* Image */}
        <div className="flex h-[200px] shrink-0 items-center justify-center bg-neutral-100 p-2 sm:h-auto sm:w-[44%] sm:min-h-[320px] sm:max-h-[min(88vh,500px)] sm:p-3">
          {prompt.image_url ? (
        ...
        {/* Content */}
        <div className="flex min-h-0 flex-1 flex-col p-4 sm:p-5 sm:pr-5">
          <div className="mb-3 shrink-0 pr-6">
            <h2 className="line-clamp-2 text-sm font-bold text-neutral-900 sm:text-base leading-snug">
              {prompt.title}
            </h2>
            <span className={cn("mt-2 inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-[10px] font-semibold sm:text-[11px]", categoryClass(prompt.category))}>
              {categoryEmoji(prompt.category)} {prompt.category}
            </span>
          </div>

          <div className="mb-3.5 min-h-0 flex-1 rounded-xl bg-neutral-50 px-3.5 py-3 ring-1 ring-neutral-100">
            <p className="line-clamp-[8] text-[11px] leading-[1.55] text-neutral-600 sm:line-clamp-[11] sm:text-xs sm:leading-[1.6]">
              {prompt.prompt_text}
            </p>
          </div>
          ...
          <button
            ...
            onClick={onCopy}
            className={cn(
              "flex w-full shrink-0 items-center justify-center gap-2 rounded-xl py-3 text-xs font-semibold transition-all duration-200 sm:py-3.5 sm:text-sm active:scale-[0.98]",
              copied
          ...
          >
            {copied ? (
              <><Check className="h-3.5 w-3.5 sm:h-4 sm:w-4" /> Copied!</>
            ) : (
              <><Copy className="h-3.5 w-3.5 sm:h-4 sm:w-4" /> Copy Prompt</>
            )}
          </button>
        </div>
      </div>
    ...
    <div className="masonry-grid">
      {Array.from({ length: 10 }).map((_, i) => (
        <div key={i} className="masonry-item animate-pulse">
          <div className={cn("rounded-2xl bg-neutral-100", PIN_ASPECTS[i % PIN_ASPECTS.length])} />
...
function EmptyState({ hasPrompts }: { hasPrompts: boolean }) {
  return (
    <div className="mx-auto max-w-md rounded-2xl border border-neutral-200 bg-white p-12 text-center shadow-sm">
      <div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-2xl bg-violet-50">
